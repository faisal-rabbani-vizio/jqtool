...
def searchYT($isVoice):
  .nextPageToken as $token | {
    rows: [
      {
        layout: "carousel",
        name: "YouTube",
        ...
        tiles: (
          .items | map(
            {
              name: (.snippet.title | htmldecode),
              app: "",
              image: .snippet.thumbnails.medium.url,
              "analytic": {
                metadata: {
                  itemTitle: (.snippet.title | htmldecode)
                }
              },
              action: {
                launch:
                {
                  id:"1",
                  ns:"5",
                  url: (if ($isVoice == false) then ("https://www.youtube.com/tv?v=" + .id.videoId + "&launch=search") else ("https://www.youtube.com/tv?v=" + .id.videoId + "&launch=search&launch_tag=voice&vs=0") end)
                }
              }
            }
          ) +
